...
Deno.serve(async (req) => {
  ...
  await adminClient.from("growth_events").insert({
    user_id: user.id,
    event_type: "CREATOR_REJECTED",
    artifact_id: normalizedArtifactId,
    run_id: runId,
    metadata: {
      reason_code: reasonCode,
  ...
  try {
    ...
    const conversationAlias = "creator_mind_preferences";
    await mindsClient.ensureConversation(conversationAlias, mindId);
    
    const memoryPayload = `
[CREATOR FEEDBACK RECEIVED]
The creator has rejected an artifact generated by the agent: ${targetAgentName || "global"}.
Reason: ${reasonCode}
Specific feedback: ${freeText || "None provided"}
Please remember this preference for future generations.
    `.trim();

    await mindsClient.sendMessage({ alias: conversationAlias, messageText: memoryPayload });
  } catch (mindsErr) {
    console.error("Failed to sync preference to Minds SDK:", mindsErr);
    // Non-fatal error, feedback was already saved to DB
  }
